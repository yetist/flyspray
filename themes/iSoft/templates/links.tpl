...
if ($user->isAnon()):
	...
	<?php if ($cas->available()) { ?>
		<?php if ($fs->prefs['anon_reg']) { ?>
			<a id="registerlink" class="active" href="https://account.isoft-linux.org/accounts/register/"><?php echo Filters::noXSS(L('register')); ?></a>
		<?php } ?>
	<a href="<?php echo Filters::noXSS($baseurl); ?>index.php?do=casauth<?php echo '&return_to=' . base64_encode($_SERVER['REQUEST_URI']);?>" accesskey="l" class="active"><?php echo Filters::noXSS(L('login')); ?></a>
	<?php }else{ ?>
